...
int main() {
    ...
    auto season_lengths = linspace<double>(1, 100, 100); 
    auto ICS = linspace<double>(0.2, 0.2, 1); 
    ...
    //std::ofstream out("ext_rate_ic.csv");
    std::ofstream out("ext_rate_ic02.csv");
    out << "IC,SeasonLength,Trajectory,FinalTime" << std::endl;

    
    // Output trajectories
    //std::ofstream traj_out("trajectories_ic.csv");
    std::ofstream traj_out("trajectories_ic02.csv");
    traj_out << "IC,SeasonLength,Trajectory,Time,x0,x1,x2,x3" << std::endl;
            ...
            for (double k_local : ICS){
                ...
                while (t < t_end) {
                    ...
                    if (propensity_func != last_func) {
                        traj_out << k_local << ',' << season_length << "," << traj << "," << t;
                        for (const auto &xi : x) {
                            traj_out << "," << xi;
                        }
                        traj_out << std::endl;
                ...
                // Write results
                out << k_local << ',' << season_length << "," << traj << "," << t << std::endl;
