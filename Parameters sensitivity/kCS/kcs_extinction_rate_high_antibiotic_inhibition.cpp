...
int main() {
    ...
    auto season_lengths = linspace<double>(10, 100, 5); 
    auto KCS = linspace<double>(0.001, 1, 10); 
    ...
    std::ofstream out("ext_rate_kcs_high_antibiotic_inhibition.csv");
    out << "CS,SeasonLength,Trajectory,FinalTime" << std::endl;

    
    // Output trajectories
    //std::ofstream traj_out("trajectories_kcs.csv");
            ...
            for (double CS_local : KCS){
                ...
                while (t < t_end) {
                    ...
                    if (propensity_func != last_func) {
                        traj_out << CS << ',' << season_length << "," << traj << "," << t;
                        for (const auto &xi : x) {
                            traj_out << "," << xi;
                        }
                        traj_out << std::endl;
                ...
                // Write results
                out << CS << ',' << season_length << "," << traj << "," << t << std::endl;
