...
int main() {
    ...
    auto season_lengths = linspace<double>(50, 50, 1); 
    auto mus = linspace<double>(0, 6, 20);
    auto ratios = linspace<double>(1,20,10);
    ...
    std::ofstream out("ext_rate_mu.csv");
    out << "mu1,R,SeasonLength,Trajectory,FinalTime" << std::endl;

    
    // Output trajectories
    std::ofstream traj_out("trajectories_mu.csv");
    traj_out << "mu1,R,SeasonLength,Trajectory,Time,x0,x1,x2,x3" << std::endl;
    ...
    //traj_plot << "SeasonLength,Trajectory,Time,x0,x1,x2,x3" << std::endl;
        ...
        for (double season_length : season_lengths) { 
            ...
            for (double muexp : mus){
                std::cout << "mu: " << muexp << std::endl;
                for (double ratio : ratios){
                    double mu1 = pow(10,-muexp);
                    double mu2 = mu1*ratio;
                    ...
                    for (int traj = 0; traj < num_trajectories; ++traj) {
                        ...
                        while (t < t_end) {
                            ...
                            if (propensity_func != last_func) {
                                traj_out << mu1 << ',' << ratio << ',' << season_length << "," << traj << "," << t;
                                for (const auto &xi : x) {
                                    traj_out << "," << xi;
                                }
                                traj_out << std::endl;
                        ...
                        // Write results
                        out << mu1 << ',' << ratio << ',' << season_length << "," << traj << "," << t << std::endl;
