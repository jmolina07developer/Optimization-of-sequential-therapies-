...
// Parameters
double mu1 = 1e-3; //mutational rate
double mu2 = 1e-2; //mutational rate
double kA = 0.5; //antibiotic inhibition
double kB = 0.5; //antibiotic inhibition
double CS = 0.05; //collateral sensitivity
double CR = 1; //cost of resistance 1 == no cost
double gammaV = 0.01; //gammaV * total poulation = death rate
...
int main() {
    ...
    int num_trajectories = 1000; // Number of trajectories per season_length
    double K = 1 / gammaV; //carrying capacity

    auto season_lengths = linspace<double>(1, 100, 100); // switching period
    ...
    //std::ofstream out("ext_rate_twoS.csv");
    std::ofstream out("ext_rate_fourS.csv");
    out << "SeasonLength,Trajectory,FinalTime" << std::endl;

    
    // Output trajectories
    std::ofstream traj_out("trajectories_fourS.csv");
    ...
    //traj_plot << "SeasonLength,Trajectory,Time,x0,x1,x2,x3" << std::endl;
        ...
        for (double season_length : season_lengths) { 
            ...
            //double t_end = 2*season_length+50;
            //double t_end_treatment = 2*season_length;

            //double t_end = 3*season_length+50;
            //double t_end_treatment = 3*season_length;

            double t_end = 4*season_length+50;
            double t_end_treatment = 4*season_length;
            ...
            for (int traj = 0; traj < num_trajectories; ++traj) {
                ...
                while (t < t_end) {
                    ...
                    auto propensity_func = current_propensity_function(t, season_length, t_end_treatment); 
                    auto last_func = current_propensity_function(t-tau, season_length, t_end_treatment); 
                    
                    
                    if (propensity_func != last_func) {
                        traj_out << season_length << "," << traj << "," << t;
                        for (const auto &xi : x) {
                            traj_out << "," << xi;
                        }
                        traj_out << std::endl;
                    ...
                    /*
                    traj_plot << season_length << "," << traj << "," << t;
                    for (const auto &xi : x) {
                        traj_plot << "," << xi;
                    }
                    traj_plot << std::endl;
                ...
                // Write results
                out << season_length << "," << traj << "," << t << std::endl;
            ...
            std::cout << "Season Length: " << season_length
                      << ", Extinction Rate: " << extinction_rate
                      << ", Average Population: " << avgN << std::endl;
