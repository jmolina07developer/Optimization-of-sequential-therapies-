...
int main() {
    ...
    auto season_lengths = linspace<double>(1, 100, 100); // switching periods
    ...
    std::ofstream out("ext_rate_oneS.csv");
    out << "SeasonLength,Trajectory,FinalTime" << std::endl;

    
    // Output trajectories
    std::ofstream traj_out("trajectories_oneS.csv");
    ...
    //traj_plot << "SeasonLength,Trajectory,Time,x0,x1,x2,x3" << std::endl;
        ...
        for (double season_length : season_lengths) { 
            ...
            double t_end = season_length+50;
            double t_end_treatment = season_length;
            ...
            for (int traj = 0; traj < num_trajectories; ++traj) {
                ...
                while (t < t_end) {
                    ...
                    if (propensity_func != last_func) { // save population trajectory before the switch
