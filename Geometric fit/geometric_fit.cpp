...
int main() {
    ...
    auto season_lengths = linspace<double>(10, 100, 10); // Tiempos de tratamiento
    //auto season_lengths = linspace<double>(150, 150, 1); // Tiempos de tratamiento
    ...
    std::ofstream out("geom_ext_rate.csv");
    out << "SeasonLength,Trajectory,FinalTime" << std::endl;

    
    // Output trajectories
    std::ofstream traj_out("geom_trajectories.csv");
    traj_out << "SeasonLength,Trajectory,Time,x0,x1,x2,x3" << std::endl;
        ...
        for (double season_length : season_lengths) { // Bucle para recorrer los valores de season_length
            ...
            double t_end_treatment = n_rounds*season_length;
            double t_end = t_end_treatment + 50;
