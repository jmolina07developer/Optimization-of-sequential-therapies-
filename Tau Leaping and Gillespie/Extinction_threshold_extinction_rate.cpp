...
int main() {
    ...
    auto Umbrales = linspace<double>(3, 8, 6); 
    ...
    std::ofstream out("ext_rate_threshold.csv");
    out << "umbral,SeasonLength,Trajectory,FinalTime" << std::endl;

    
    // Output trajectories
    std::ofstream traj_out("trajectories_threshold.csv");
    traj_out << "umbral,SeasonLength,Trajectory,Time,x0,x1,x2,x3" << std::endl;
            ...
            for (double u : Umbrales){
                ...
                while (t < t_end) {
                    ...
                    if (propensity_func != last_func) {
                        traj_out << u << ',' << season_length << "," << traj << "," << t;
                        for (const auto &xi : x) {
                            traj_out << "," << xi;
                        }
                        traj_out << std::endl;
                ...
                // Write results
                out << u << ',' << season_length << "," << traj << "," << t << std::endl;
