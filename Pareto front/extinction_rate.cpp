...
int main() {
    ...
    std::ofstream out("ext_rate_n10000_time300_less.csv");
    out << "Rounds,SeasonLength,Trajectory,FinalTime" << std::endl;

    
    // Output trajectories
    std::ofstream traj_out("finalpop_n10000_time300_less.csv");
    traj_out << "Rounds,SeasonLength,Trajectory,Time,final,x0,x1,x2,x3" << std::endl;
    ...
    //traj_plot << "SeasonLength,Trajectory,Time,x0,x1,x2,x3" << std::endl;
        ...
        for (double season_length : season_lengths) { 
            ...
            double avgN = 0.0;
            round = 1;
            while (season_length*round < t_end_treatment){
                ...
                // Run trajectory
                while (t < season_length*round + 50) {
                    // Determine the current propensity function based on the season
                    auto propensity_func = current_propensity_function(t, season_length, season_length*round); 
                    auto last_func = current_propensity_function(t-tau, season_length, season_length*round); 
                    ...
                    if ((t < season_length*round+0.01)&(t >= season_length*round-0.1)){
                        traj_out << round << "," << season_length << "," << traj << "," << t << ',' << 0;
                        for (const auto &xi : x) {
                            traj_out << "," << xi;
                        }
                        traj_out << std::endl;
                ...
                traj_out << round << "," << season_length << "," << traj << "," << t << ',' << 1;
                for (const auto &xi : x) {
                    traj_out << "," << xi;
                }
                traj_out << std::endl;
                ...
                // Write results
                out << round << "," << season_length << "," << traj << "," << t << std::endl;
                
                std::cout << "time " << t << " round " << round << " season " << season_length << std::endl;
