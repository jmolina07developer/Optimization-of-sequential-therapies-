...
int main() {
    ...
    // std::ofstream out("x3_app_kCS010.csv");
    ...
    std::ofstream traj_plot("traj_plot_ratiox3_x290_cs005.csv");
    traj_plot << "SeasonLength,Trajectory,Time,x0,x1,x2,x3" << std::endl;
        ...
        for (double season_length : season_lengths) { 
            ...
            while (cont < num_trajectories) {
                ...
                // Initial state
                std::vector<double> x = {00.0, 00.0, 90.0, 0.0};
                double t = t_start;
                int extinct = 0;
                int Ntot = 0;
