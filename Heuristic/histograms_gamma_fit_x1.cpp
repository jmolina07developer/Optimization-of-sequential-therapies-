...
int main() {
    ...
    auto season_lengths = linspace<double>(1000, 1000, 1); 
    auto x1lim = linspace<double>(80, 80, 1); 
    ...
    std::ofstream out("x1_dom.csv");
    out << "x1lim,Trajectory,FinalTime" << std::endl;
    ...
    //traj_plot << "SeasonLength,Trajectory,Time,x0,x1,x2,x3" << std::endl;
        ...
        for (double season_length : season_lengths) { 
            ...
            for (int x1l : x1lim){
                int cont = 0;
                int traj = 0;
                while (cont < num_trajectories){
                    ++cont;
                    ++traj;
                // Initial state
                std::cout << "Traj: " << cont << std::endl;
                std::vector<double> x = {0.0, 0.0, 80.0, 0.0};
                double t = t_start;
                int extinct = 0;
                int Ntot = 0;
